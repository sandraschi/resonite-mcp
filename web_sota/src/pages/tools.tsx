import { Wrench, ChevronDown, ChevronRight, Info, Search, Cpu } from 'lucide-react';
...
async function fetchTools(): Promise<Tool[]> {
    try {
        ...
        const toolsList = (data as { tools?: Tool[] }).tools || data.result?.tools || [];
        return toolsList;
    } catch (err) {
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {isLoading && (
                <div className="grid gap-4">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="h-20 bg-muted/20 animate-pulse rounded-2xl glass border border-border/30"></div>
                    ))}
                </div>
            )}
            ...
            {isError && (
                <div className="p-8 text-center bg-red-500/5 border border-red-500/20 rounded-2xl">
                    ...
                    <p className="text-sm font-bold text-red-400 uppercase tracking-widest">Protocol Retrieval Failed</p>
                    <p className="text-xs text-muted-foreground mt-1">Unable to fetch tool metadata from the MCP gateway.</p>
                </div>
            )}
            ...
            {Object.entries(categories).length === 0 && !isLoading && !isError && (
                <div className="p-16 text-center">
                    ...
                    <p className="text-sm text-muted-foreground italic">No tools matching your query found in the neural cache.</p>
                </div>
            )}
                ...
                {Object.entries(categories).map(([cat, catTools]) => (
                    <div key={cat} className="space-y-4">
                        <div className="flex items-center gap-3 px-1">
                            ...
                            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/70">{cat} Systems</h3>
                        ...
                        <div className="grid gap-3">
                            {catTools.map(tool => (
                                <Card key={tool.name} className={cn(
                                    ...
                                    expanded === tool.name && "border-indigo-500/40 bg-indigo-500/[0.02]"
                                )}>
                                    <button
                                    ...
                                    >
                                        <div className="flex items-start gap-4">
                                            <div className={cn(
                                                ...
                                                expanded === tool.name ? "bg-indigo-500/10 border-indigo-500/30 text-indigo-400" : "group-hover:bg-muted"
                                            )}>
                                            ...
                                            <div className="p-4 rounded-xl bg-black/20 border border-white/[0.03] space-y-4">
                                                ...
                                                {tool.parameters && (
                                                    <div className="space-y-3 pt-2">
                                                        <div className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-2">Parameters</div>
                                                        <div className="grid gap-2">
                                                            {Object.entries(tool.parameters).map(([pname, param]) => (
                                                                <div key={pname} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 p-3 rounded-lg bg-muted/20 border border-white/[0.02]">
                                                                    <code className="text-xs font-bold text-indigo-300 w-full sm:w-40 flex-shrink-0">{pname}</code>
                                                                    <div className="flex items-center gap-3 flex-1">
                                                                        <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground font-mono uppercase font-black">{param.type}</span>
                                                                        <span className="text-xs text-muted-foreground italic shrink">
                                                                            {param.description || 'No documentation provided.'}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                            ))}
