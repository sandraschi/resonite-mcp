...
export function Contacts() {
    ...
    const contacts = (data ?? []).filter(c =>
        (tab === 'all' || c.userStatus?.onlineStatus?.toLowerCase() === 'online') &&
        ...
        <div className="space-y-6 page-enter">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500">Your Resonite friends &amp; social connections</p>
                </div>
            </div>
            ...
            <div className="space-y-2">
                    ...
                    return (
                        <div key={c.id} className="glass-card p-3 flex items-center gap-3">
                            ...
                            <div className="relative flex-shrink-0">
                                {c.profile?.iconUrl ? (
                                    <img src={c.profile.iconUrl} alt={c.contactUsername ?? 'contact'} className="w-10 h-10 rounded-full object-cover" />
                                ) : (
                                ...
                                <span className={cn('absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-slate-900', statusColor(online))} aria-label={online ?? 'offline'} />
                            </div>

                            {/* Info */}
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-slate-100 truncate">{c.contactUsername ?? c.id}</p>
                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                    {inSession && (
                                        <>
                                            <Globe2 className="w-3 h-3 text-indigo-400" aria-hidden="true" />
                            ...
                            <div className="flex-shrink-0">
                                {c.isAccepted
                                    ? <UserCheck className="w-4 h-4 text-emerald-500" aria-label="Friend" />
                                    : <UserX className="w-4 h-4 text-slate-600" aria-label="Pending" />}
                            </div>
                            ...
                            {inSession && (
                                <button
                                    ...
                                    onClick={() => navigator.clipboard.writeText(`res-session:///${c.userStatus?.currentSessionId ?? ''}`)}
                                >
                ...
                {contacts.length === 0 && !isLoading && !isError && (
                    <div className="text-center py-12 text-slate-600">
                        ...
                        <p className="text-sm">{tab === 'online' ? 'No contacts online right now' : 'No contacts found'}</p>
                    </div>
                )}
            </div>
        </div>
