import { ShoppingBag, Search, Star, Download, Heart, Share2, Tag, Sparkles, Flame, Clock, ChevronRight } from 'lucide-react';
...
const MOCK_ITEMS: MarketplaceItem[] = [
    {
        ...
        author: 'ResoniteSOTA',
        description: 'Premium avatar base with full facial tracking and ProtoFlux logic.',
        price: 'Free',
        rating: 4.9,
        downloads: 1240,
        category: 'Avatars',
        ...
        <div className="space-y-6 page-enter">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white">Resonite Marketplace</h2>
                        <p className="text-sm text-slate-500">Discover and install community-driven plugins, avatars, and worlds</p>
                    </div>
                </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {['All', 'Avatars', 'Worlds', 'Tools', 'Skins', 'Plugins', 'Audio'].map(category => (
                    <button
                        key={category}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {filteredItems.map((item) => (
                    <div key={item.id} className="group glass-card rounded-2xl overflow-hidden flex flex-col border border-white/[0.05] hover:border-emerald-500/30 hover:bg-emerald-500/[0.02] transition-all">
                        <div className="aspect-video bg-slate-900 relative">
                            ...
                            <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button title="Like item" aria-label={`Like ${item.name}`} className="p-2 bg-black/60 text-white rounded-lg backdrop-blur-md hover:bg-black/80 transition-colors">
                                    <Heart className="w-4 h-4" />
                                </button>
                                <button title="Share item" aria-label={`Share ${item.name}`} className="p-2 bg-black/60 text-white rounded-lg backdrop-blur-md hover:bg-black/80 transition-colors">
                        ...
                        <div className="p-4 flex-1 flex flex-col">
                            <div className="flex items-start justify-between gap-2 mb-1">
                                <h3 className="font-bold text-white group-hover:text-emerald-400 transition-colors line-clamp-1">{item.name}</h3>
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-[10px] text-slate-600">({item.downloads})</span>
                                </div>
                                <span className="text-[10px] text-slate-500">by <span className="text-slate-300">{item.author}</span></span>
                            </div>
                ...
                {filteredItems.length === 0 && (
                    <div className="col-span-full py-20 text-center space-y-4">
                        ...
                        <div>
                            <p className="text-xl font-bold text-slate-500">No treasures found here</p>
                            <p className="text-slate-600">Try adjusting your filters or search terms</p>
                        </div>
                        <button onClick={() => { setSearchQuery(''); setActiveCategory('All'); }} className="text-emerald-400 font-bold hover:underline">Clear all filters</button>
                    </div>
                )}
            </div>
