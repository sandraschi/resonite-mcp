        ...
        <div className="space-y-6">
            <header className="flex flex-col gap-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-foreground">Avatar Control</h1>
                        <p className="text-sm text-muted-foreground">Virtual joysticks for mobile or desktop remote control.</p>
                    </div>
                </div>
            </header>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="lg:col-span-3 rounded-2xl border border-border bg-card/30 p-4 backdrop-blur-md">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                            <h4 className="text-sm font-semibold text-foreground">ProtoFlux Setup Required</h4>
                            <p className="text-xs text-muted-foreground leading-relaxed">
                                To use these controls, your avatar must have a ProtoFlux setup listening for 'MoveX', 'MoveY', and 'ThirdPerson' parameters.
                                See the <a href="/help" className="text-indigo-400 hover:underline">Help page</a> for the circuit diagram.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-3xl border border-border bg-card/50 p-8 flex flex-col items-center justify-center gap-6 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full">
                        <div className="rounded-xl bg-black/30 p-3 border border-white/5">
                            <span className="block text-[10px] uppercase tracking-wider text-muted-foreground">Axis X</span>
                            <span className="text-sm font-mono text-indigo-400 font-bold">{lastMove.x.toFixed(2)}</span>
                        </div>
                        <div className="rounded-xl bg-black/30 p-3 border border-white/5">
                            <span className="block text-[10px] uppercase tracking-wider text-muted-foreground">Axis Y</span>
                            <span className="text-sm font-mono text-indigo-400 font-bold">{lastMove.y.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-3xl border border-border bg-card/50 p-8 flex flex-col items-center justify-center gap-6 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="flex flex-col gap-2 w-full">
                        <button
                            onClick={toggleView}
                            className={`flex w-full items-center justify-center gap-2 rounded-xl py-3 px-4 font-semibold transition-all duration-300 border ${viewType === 'third-person'
                        ...
                        >
                            {viewType === 'first-person' ? <Eye className="h-5 w-5" /> : <EyeOff className="h-5 w-5" />}
                            {viewType === 'first-person' ? 'Switch to Third Person' : 'Switch to First Person'}
                        </button>
                    </div>
                </div>
                ...
                <div className="rounded-3xl border border-border bg-card/50 p-8 flex flex-col gap-6 shadow-2xl">
                    <h3 className="text-lg font-bold text-foreground">Control Settings</h3>
