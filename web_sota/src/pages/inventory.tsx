...
async function fetchRecords(
	_path: string,
): Promise<{ records: Record[]; warning?: string }> {
	const params = new URLSearchParams({ limit: "100", offset: "0" });
	const r = await fetch(apiUrl(`/api/resonite/inventory/list?${params}`));
	if (!r.ok) {
	...
	// "warning" status (e.g. OSC timeout, no in-world responder answering) is
	// NOT an error -- it's the honest "nothing came back" case. Surface the
	// message but don't throw, so the empty-state UI (not the error UI) shows.
	return {
		records: data.items ?? [],
		warning: data.status === "warning" ? data.message : undefined,
	};
}
...
export function Inventory() {
	...
	const warning = fetchResult?.warning;
	...
	return (
		<div className="space-y-8 animate-in fade-in duration-700">
			...
			<div className="relative group">
				<div className="absolute -inset-1 bg-gradient-to-r from-purple-500/20 to-indigo-500/20 rounded-lg blur opacity-25 group-hover:opacity-50 transition duration-1000" />
						...
						<button
							...
							className={cn(
								...
								i === breadcrumbs.length - 1
									? "text-purple-400 pointer-events-none"
						...
						<div className="flex-1">
							...
							<p className="text-[11px] text-amber-500/70 font-medium">
								{error?.message === "AUTH_REQUIRED"
									...
									: error?.message ||
										"Unknown matrix error occurred during synchronization."}
							</p>
						</div>
						<button
							type="button"
			...
			{isLoading && (
				<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 animate-pulse">
					{Array.from({ length: 12 }, (_, i) => `inv-skel-${i}`).map((id) => (
						<div
							key={id}
							className="aspect-square rounded-xl bg-card/40 border border-white/5"
						/>
					))}
			...
			{!isLoading &&
				...
				(records && records.length > 0 ? (
					...
					>
						{records.map((rec) => (
							// Card wraps per-record action buttons: cannot be a <button> itself.
							...
							<div
								...
								tabIndex={0}
								aria-label={
									rec.recordType === "directory" ? `Open ${rec.name}` : rec.name
								}
								onClick={() =>
									rec.recordType === "directory" &&
									handleNavigate(rec.path ?? `${path}/${rec.name}`)
								}
								onKeyDown={(e) => {
									if (
										(e.key === "Enter" || e.key === " ") &&
							...
							>
								...
								>
									<h3
										className="text-[11px] font-bold text-foreground truncate group-hover:text-purple-400 transition-colors"
										title={rec.name}
									>
										{rec.name}
									</h3>
					...
					<div className="flex flex-col items-center justify-center py-20 bg-card/20 backdrop-blur-sm border border-dashed border-white/10 rounded-2xl gap-4">
						...
						<div className="text-center">
							<p className="text-sm font-bold text-foreground">
								Record Repository Empty
							</p>
							<p className="text-[11px] text-muted-foreground max-w-[280px] mx-auto mt-1">
								{warning
									? warning
