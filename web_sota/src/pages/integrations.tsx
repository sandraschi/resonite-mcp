...
export function Integrations() {
	...
	const worldlabsMutation = useMutation({
		mutationFn: async () => {
			const resp = await fetch(apiUrl("/api/integrations/worldlabs/import"), {
				method: "POST",
			});
	...
	const blenderMutation = useMutation({
		mutationFn: async () => {
			const resp = await fetch(apiUrl("/api/integrations/blender/import"), {
				method: "POST",
			});
			if (!resp.ok) throw new Error(await resp.text());
			return resp.json();
	...
	const unityMutation = useMutation({
		mutationFn: async () => {
			const resp = await fetch(apiUrl("/api/integrations/unity/avatar"), {
				method: "POST",
			});
			if (!resp.ok) throw new Error(await resp.text());
			return resp.json();
				...
				<div className="flex gap-4 items-start">
					...
					<div>
						...
						<p className="text-sm text-slate-500 mb-4">
							The Resonite MCP is automatically detecting compatible servers in
							your local fleet. Ensure WorldLabs, Blender, and Unity3D MCP
							servers are running for these workflows to activate.
						</p>
						<div className="flex flex-wrap gap-2">
							<span className="px-2 py-1 rounded bg-white/[0.04] border border-white/[0.08] text-[10px] uppercase tracking-wider text-slate-400 font-bold">
								worldlabs-mcp: active
							</span>
							<span className="px-2 py-1 rounded bg-white/[0.04] border border-white/[0.08] text-[10px] uppercase tracking-wider text-slate-400 font-bold">
								blender-mcp: active
							</span>
