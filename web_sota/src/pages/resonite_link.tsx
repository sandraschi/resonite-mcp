...
export function ResoniteLinkPage() {
  ...
  const connectMutation = useMutation({
    mutationFn: () => connectRL(host, Number.parseInt(port, 10)),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["rl-status"] }),
  });

  const discoverMutation = useMutation({
    mutationFn: discoverRL,
  });
  ...
  const handleReadField = async () => {
    ...
    setFieldResult(null);
    try {
      setFieldResult(await readField(fieldRef));
    } catch (e) {
  ...
  const handleGetChildren = async () => {
    ...
    try {
      setChildrenResult(
        await getChildren(
          slotRef ||
            (status?.session_info as Record<string, string> | null)
              ?.root_slot_id ||
            "",
        ),
      );
    } catch (e) {
      setChildrenError((e as Error).message);
    }
  };
  ...
  const handleReflect = async () => {
    ...
    try {
      setReflectResult(await reflectTypes(reflectType || undefined));
    } catch (e) {
      setReflectError((e as Error).message);
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mt-1 opacity-70 flex items-center gap-2">
                ...
                WebSocket Data Model Bridge · Protocol v0.13.1
              </p>
            </div>
          </div>
        ...
        <Card className="border-white/10 bg-card/30 backdrop-blur-md glass">
          <CardContent className="p-5 space-y-4">
            ...
            >
              ...
              Discover Sessions (recommended — don't guess a port)
            </button>
            {discoverMutation.isSuccess && (
              <div className="space-y-1.5">
                {discoverMutation.data.sessions.length === 0 ? (
                  <p className="text-[11px] text-muted-foreground italic">
                    No sessions found. Is Resonite running with ResoniteLink
                    enabled (Dash → Session → Settings)?
                  </p>
                ) : (
                  discoverMutation.data.sessions.map((s) => (
                    <button
                      key={s.sessionID}
                      type="button"
                      onClick={() => {
                        setHost(s.host === "0.0.0.0" ? "localhost" : s.host);
                        setPort(String(s.linkPort));
                      }}
            ...
            <p className="text-[10px] text-muted-foreground italic">
              Or fill in manually (fallback only — the port below is a guess
              until you discover or connect successfully):
            </p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label
                  htmlFor="rl-host"
                  className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider block mb-1"
                >
                  Host
                </label>
                <input
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="discover first, or e.g. 11831"
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="border-white/10 bg-card/30 backdrop-blur-md glass">
          <CardContent className="p-5 space-y-3">
            ...
            {fieldError && (
              <p className="text-[10px] text-red-400">{fieldError}</p>
            )}
        ...
        <Card className="border-white/10 bg-card/30 backdrop-blur-md glass">
          <CardContent className="p-5 space-y-3">
            ...
            {childrenError && (
              <p className="text-[10px] text-red-400">{childrenError}</p>
            )}
      ...
      <Card className="border-white/10 bg-card/30 backdrop-blur-md glass">
        <CardContent className="p-5 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-bold text-foreground uppercase tracking-widest">
              Reflect — Component Type Discovery
            </p>
          </div>
          ...
          {reflectError && (
            <p className="text-[10px] text-red-400">{reflectError}</p>
          )}
      ...
      <Card className="border-white/10 bg-card/20 backdrop-blur-sm">
        <CardContent className="p-5 space-y-2">
          <p className="text-xs font-bold text-foreground uppercase tracking-widest opacity-70">
            HTTP API Quick Reference
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              ["GET", "/rl/discover", "Find sessions on LAN (do this first)"],
              ["POST", "/rl/connect", "Connect with {host, port}"],
              ["POST", "/rl/disconnect", "Graceful disconnect"],
              ["GET", "/rl/status", "Connection + session info"],
              ["GET", "/rl/field/{ref_id}", "Read a component's full data"],
              [
                "POST",
                "/rl/field",
                "Write a component member (requires 'member')",
              ],
              ...
              >
                ...
                <span className="text-muted-foreground ml-auto text-right hidden sm:block">
                  {desc}
                </span>
              </div>
            ))}
