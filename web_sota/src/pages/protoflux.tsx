...
function CodeBlock({ code, lang = "" }: { code: string; lang?: string }) {
	return (
		<div className="relative group">
			<pre
				className={`bg-black/40 border border-white/[0.06] rounded-xl p-4 text-xs font-mono text-slate-300 overflow-x-auto leading-relaxed language-${lang}`}
			>
				{code}
			</pre>
...
export function ProtoFluxPage() {
	...
	const connectMut = useMutation({
		mutationFn: (f: ConnectForm) =>
			fetch(apiUrl("/rl/connect"), {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ host: f.host, port: f.port }),
			}).then((r) => r.json()),
	...
	const disconnectMut = useMutation({
		mutationFn: () =>
			fetch(apiUrl("/rl/disconnect"), { method: "POST" }).then((r) => r.json()),
		onSuccess: () => qc.invalidateQueries({ queryKey: ["rl-status"] }),
	});
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<h2 className="text-xl font-bold text-white">
							ProtoFlux & ResoniteLink
						</h2>
						<p className="text-sm text-slate-500">
							In-world setup guide + live data model control
						</p>
					</div>
				</div>
				<div
				...
				<div className="space-y-6">
					...
					<div className="glass-card p-5 space-y-3">
						...
						<p className="text-sm text-slate-400 leading-relaxed">
							ResoniteLink is Resonite's official WebSocket data-model bridge
							(v0.8.3, Feb 2026). It lets external tools read and write any
							field in the world scene graph by{" "}
							<span className="text-white font-medium">ref ID</span>. ProtoFlux
							logix nodes expose gameplay logic that you can trigger from
							outside. This MCP server bridges HTTP → ResoniteLink so your AI
							tools, scripts, and dashboards can control what happens inside the
							world.
						</p>
						<div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2">
							<AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" />
							ResoniteLink must be enabled per-world by the host. Only the host
					...
					<div className="glass-card p-6 space-y-5">
						...
						<div className="space-y-5">
							<Step n={1} title="Open the Sessions panel">
								<p>
									Open your Resonite dash →{" "}
									<span className="text-white">Sessions</span> tab. Find your
									world in the list (or the world you're currently hosting).
								</p>
							</Step>
							<Step n={2} title='Click "Enable ResoniteLink"'>
								<p>
									On the world entry, click the{" "}
								...
								<CodeBlock
									lang="json"
									code={`"enableResoniteLink": true,\n"forceResoniteLinkPort": 4242`}
								/>
							</Step>
							<Step n={3} title="Set permissions">
								<p>
									From v0.8.x, you can also add a{" "}
									<span className="text-white font-mono text-[11px] bg-white/10 px-1.5 py-0.5 rounded">
										ResoniteLinkPermissions
									</span>{" "}
									component to your world root slot to control who can
									read/write. Defaults to host/admin only.
								</p>
							</Step>
						</div>
					</div>
					...
					<div className="glass-card p-6 space-y-5">
						...
						<p className="text-xs text-slate-400 leading-relaxed">
							ResoniteLink writes to fields by <em>ref ID</em>. The easiest way
							to expose a controllable parameter is to attach a{" "}
							<span className="text-white font-mono text-[11px]">
								DynamicValueVariable&lt;T&gt;
							</span>{" "}
							component and wire it into your ProtoFlux graph. Then read the ref
							ID via the inspector and paste it here.
						</p>
						...
						<div className="space-y-5">
							<Step n={1} title="Create a control slot">
								<p>
									...
									<span className="text-white">_MCPControls</span>.
								</p>
							</Step>
							...
							<Step n={2} title="Add a DynamicValueVariable for each parameter">
								<p>
									Attach a{" "}
									<span className="text-white font-mono text-[11px] bg-white/10 px-1.5 py-0.5 rounded">
										DynamicValueVariable&lt;float3&gt;
									</span>
									(for position/scale) or{" "}
									<span className="text-white font-mono text-[11px] bg-white/10 px-1.5 py-0.5 rounded">
										DynamicValueVariable&lt;bool&gt;
									</span>
									(for toggles) to that slot.
								</p>
								<p>
									Give each a unique{" "}
									<span className="text-white">VariableName</span>, e.g.{" "}
									<span className="text-white font-mono text-[11px]">
										MCP/TeleportTarget
									</span>
									.
							...
							<Step n={3} title="Wire into ProtoFlux">
								<p>
									In your ProtoFlux graph, use a{" "}
									<span className="text-white font-mono text-[11px] bg-white/10 px-1.5 py-0.5 rounded">
										Read DynamicVariable
									</span>{" "}
									node pointing at your variable name. Connect its output to
									whatever you want to control — teleport target, animation
									speed, colour, etc.
								</p>
								<p>
									For triggerable actions, use{" "}
									<span className="text-white font-mono text-[11px] bg-white/10 px-1.5 py-0.5 rounded">
										DynamicImpulseReceiver
									</span>
									and trigger it by writing a bool field to true.
								</p>
							</Step>

							<Step n={4} title="Find the ref ID">
								<p>
									Open the Inspector on your DynamicValueVariable component. The{" "}
									<span className="text-white">ID</span> shown at the top of the
									inspector panel is the <strong>ref ID</strong>
									you'll use in ResoniteLink write calls.
								</p>
								<p>
									It looks like:{" "}
									<span className="text-white font-mono text-[11px] bg-white/10 px-1.5 py-0.5 rounded">
							...
							<Step n={5} title="Test a write from the Live Control tab">
								<p>
									Connect to ResoniteLink below, then switch to the{" "}
									<span className="text-white">Live Control</span> tab. Paste
									the ref ID and write a value — you should see the effect
									in-world immediately.
								</p>
							</Step>
						</div>
					</div>
					...
					<div className="glass-card p-6 space-y-4">
						...
						<p className="text-xs text-slate-400">
							Complete ProtoFlux setup to allow external teleport writes via
							ResoniteLink:
						</p>
						<CodeBlock
							lang="text"
							code={`World slot layout:
  _MCPControls
				...
				<div className="space-y-5">
					...
					<div className="glass-card p-5 space-y-4">
						...
						<div className="flex gap-3 items-end flex-wrap">
							...
							<button
								onClick={() =>
									qc.invalidateQueries({ queryKey: ["rl-status"] })
								}
						...
						{connectMut.isError && (
							<p className="text-xs text-rose-400">
								Connection failed. Is Resonite running with ResoniteLink
								enabled?
							</p>
						)}
						{connectMut.isSuccess && !isConnected && (
							<p className="text-xs text-rose-400">
								Got a response but status shows disconnected — check server
								logs.
							</p>
						)}

						{isConnected &&
							rlStatus?.session_info &&
							Object.keys(rlStatus.session_info).length > 0 && (
					...
					{!isConnected && (
						<div className="glass-card p-10 text-center space-y-2">
							...
							<p className="text-slate-500 font-medium">
								Connect to ResoniteLink to access live controls
							</p>
							<p className="text-xs text-slate-600">
								Resonite must be running and ResoniteLink enabled in the world
							</p>
						</div>
					)}
				</div>
...
function FieldPanel() {
	...
	return (
		<div className="glass-card p-5 space-y-4">
			...
			{readError && (
				<p className="text-xs text-rose-400 font-mono">{readError}</p>
			)}
			{readResult !== null && (
			...
			<div className="border-t border-white/[0.05] pt-4 space-y-3">
				...
				{writeMut.isSuccess && (
					<p className="text-xs text-emerald-400">Write OK</p>
				)}
				{writeMut.isError && (
					<p className="text-xs text-rose-400">
	...
	return (
		<div className="glass-card p-5 space-y-4">
			...
			<p className="text-xs text-slate-500">
				Leave type blank to list all supported components. Enter a type to list
				its fields.
			</p>
			<div className="flex gap-3 items-end flex-wrap">
