...
## 🌍 WorldLabs Marble Import with ProtoFlux

Import Gaussian splats generated by [worldlabs-mcp](https://github.com/sandraschi/worldlabs-mcp)
into Resonite via OSC-triggered ProtoFlux graphs.

### OSC Import Protocol

worldlabs-mcp sends OSC messages to Resonite at `/worldlabs/import` with
3 string arguments: `splat_url`, `mesh_url`, `world_name`.

### ProtoFlux Graph Setup

To receive and import splats automatically:

1. Open Resonite and enter the ProtoFlux editor
2. Create a new graph with these nodes:

```
[OSC Data Input]          ← address: /worldlabs/import, port: 9000
       │
       ▼
[String Split]            ← delimiter: ","
       │
       ├── output[0] ──► [HTTP GET] ──► [Import Splat] ──► Slot: root
       │
       └── output[1] ──► (mesh URL, optional)
       │
       └── output[2] ──► (world name, for display)
```

3. Save the graph as a reusable ProtoFlux asset
4. Keep Resonite running on port 9000 (OSC default)

### Testing the Import

1. Generate a world in worldlabs-mcp
2. Click "Resonite" in the Asset Panel under Smart Handoff
3. worldlabs-mcp downloads the splat and sends OSC to Resonite
4. The ProtoFlux graph receives the message and imports the splat
...
imports splats via ResoniteLink when it receives a `/worldlabs/import` message.

### Troubleshooting

| Issue | Fix |
|-------|-----|
| OSC not received | Ensure Resonite is listening on port 9000 |
| Splat imports but doesn't render | SPZ format may need conversion via spz-to-ply |
| ProtoFlux graph not triggering | Check the OSC address matches `/worldlabs/import` exactly |
| Multiple imports stacking | Resonite imports are sequential; wait for each to finish |

---

*This guide covers WorldLabs Marble integration as of worldlabs-mcp v0.5.0 and resonite-mcp v0.1.1.*
